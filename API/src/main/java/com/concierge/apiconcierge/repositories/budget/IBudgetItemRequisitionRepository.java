...
@Repository
public interface IBudgetItemRequisitionRepository extends JpaRepository<BudgetItemRequisition, UUID> {

    @Query(value = "SELECT * FROM `tb_budget_requisition` WHERE company_id=?1 AND resale_id=?2 AND budget_id=?3 order by ordem",
            nativeQuery = true)
    List<BudgetItemRequisition> listAllRequisition(Integer companyId, Integer resaleId, Integer budgetId);
